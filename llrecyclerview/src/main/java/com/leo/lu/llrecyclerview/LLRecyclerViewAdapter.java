 ...
 */
public abstract class LLRecyclerViewAdapter<VH extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<VH>  {
    protected Handler timer = new Handler();
    protected LLRecyclerView.CustomRelativeWrapper customHeaderView = null;
     ...
     */
    private int loadMoreSettingsWatch = 0;
     ...
     */
    public void setCustomHeaderView(LLRecyclerView.CustomRelativeWrapper customHeaderView) {
        this.customHeaderView = customHeaderView;
        customHeader = true;
    }

    public LLRecyclerView.CustomRelativeWrapper getCustomHeaderView() {
    ...
    private class delayEnableLoadMore implements Runnable {
        ...
        @Override
        public void run() {
            if (!enabled && loadMoreSettingsWatch > 0 && customLoadMoreView != null) {
    ...
    protected boolean detectDispatchLoadMoreDisplay(final int data_size_before_remove, final int display_size_before_remove) {
        if (data_size_before_remove == 0) {
            if (display_size_before_remove == 2) {
                ...
                if (mEmptyViewPolicy == LLRecyclerView.EMPTY_KEEP_HEADER_AND_LOARMORE) {

                } else if (mEmptyViewPolicy == LLRecyclerView.EMPTY_KEEP_HEADER) {
                    removeDispatchLoadMoreView();
                } else if (mEmptyViewPolicy == LLRecyclerView.EMPTY_CLEAR_ALL) {
                    removeDispatchLoadMoreView();
                }
                ...
                if (mEmptyViewPolicy == LLRecyclerView.EMPTY_KEEP_HEADER_AND_LOARMORE) {
                    ...
                    notifyDataSetChanged();
                } else if (mEmptyViewPolicy == LLRecyclerView.EMPTY_SHOW_LOADMORE_ONLY) {
                    notifyDataSetChanged();
                }
                return true;
            } else {
    ...
    protected void notifyAfterRemoveAllData(final int data_size_before_remove, final int display_size_before_remove) {
        try {
            ...
            final int notify_n_plus = hasHeaderView() ? data_size_before_remove + 1 : data_size_before_remove;
            ...
            if (mEmptyViewPolicy == LLRecyclerView.EMPTY_KEEP_HEADER_AND_LOARMORE) {
                ...
                notifyItemRangeRemoved(0, notify_n_plus);
                removeDispatchLoadMoreView();
            } else if (mEmptyViewPolicy == LLRecyclerView.EMPTY_SHOW_LOADMORE_ONLY) {
                notifyItemRangeRemoved(0, notify_n_plus);
                revealDispatchLoadMoreView();
            ...
            String o = e.fillInStackTrace().getCause().getMessage();
            Log.d("fillInStackTrace", o + " : ");
        }
    }
